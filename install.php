...
<?php
	...
	die("<b>FATAL:</b>The provided database name in res/php/config.php,"
		...
		. "\"" . GASALYRICS_DBNAME . "\" is incorrect or cannot be reached.");
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		mysql_query($sqlAlbum		) or die($sqlError . mysql_error() . "<br><br><b>SQL:</b>" . $sqlAlbum		);
		mysql_query($sqlLanguages	) or die($sqlError . mysql_error() . "<br><br><b>SQL:</b>" . $sqlLanguage	);
		mysql_query($sqlLyrics		) or die($sqlError . mysql_error() . "<br><br><b>SQL:</b>" . $sqlLyrics		);
		mysql_query($sqlSongs		) or die($sqlError . mysql_error() . "<br><br><b>SQL:</b>" . $sqlSongs		);
		mysql_query($sqlSymlink		) or die($sqlError . mysql_error() . "<br><br><b>SQL:</b>" . $sqlSymlink	);
...
<form id="songToAdd" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="POST" >
	...
	<br>
	<button type="submit">Proceed with the installation of GasaLyrics.</button>
</form>
<h2>Details</h2>
<p>
